...
def test_construct_search_query():
    sql = construct_search_query(
        search_query="LAST FIRST", search_type="mailing_address"
    )
    assert "LIKE '%LAST FIRST%'" in sql.upper()

    sql = construct_search_query(
        search_query="LAST FIRST", search_type="owner", search_method="starts with",
    )
    assert "LIKE 'LAST FIRST%'" in sql.upper()
    ...
    with pytest.raises(SearchTypeNotImplementedError):
        construct_search_query(
            search_query="LAST FIRST", search_type="abc", search_method="contains",
        )
